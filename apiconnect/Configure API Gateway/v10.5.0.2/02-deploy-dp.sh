...
for ((CUR_DP_SEQ=0; CUR_DP_SEQ<$NUM_OF_DPS; CUR_DP_SEQ++)); do
    ...
    if [[ ! $CUR_DP_VERSION == *"$DP_VERSION" ]]; then
        CUR_DP_HOSTNAME="$(getIndirectValue DP_MGMT_HOSTNAME_SERVER $CUR_DP_SEQ)"
        log_error "DataPower $CUR_DP_HOSTNAME firmware version $CUR_DP_VERSION does not match $DP_VERSION"
        exit
    fi
done
